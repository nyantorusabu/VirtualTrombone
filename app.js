...
let streamDestination, mediaRecorder, recordedChunks = [];

let isInitialized = false, isPlaying = false, isMicInitialized = false;
let analyser, currentStream, microphone;

// --- State ---
let currentEngine = 'real'; 
let inputMode = 'touch', slideMode = 'fixed', lipMode = 'standard', lipDir = 'normal';
let useTouchPressure = false, useYVol = false, useVibration = true;
...
const partialNames = ["", "Bb1", "Bb2", "F3", "Bb3", "D4", "F4", "Ab4", "Bb4", "C5", "D5", "Eb5", "F5", "G5", "Ab5", "Bb5", "C6"];
...
function saveSettings() {
  const settings = {
    engine: currentEngine, mode: inputMode,
    ...
    slideMode: slideMode, lipMode: lipMode, lipDir: lipDir,
    lipSens: dom.lipSens.value, yVol: dom.yVolSelect.value
  };
...
function loadSettings() {
  ...
  if (saved) {
    try {
      ...
      if (s.engine) { currentEngine = s.engine; dom.engineSelect.value = s.engine; }
      if (s.mode) { inputMode = s.mode; dom.modeSelect.value = s.mode; }
      if (s.simpleVol) { dom.simpleVol.value = s.simpleVol; document.getElementById('val-simple-vol').innerText = s.simpleVol; }
      if (s.recordTrigger) { recordTrigger = s.recordTrigger; dom.recordTriggerSelect.value = s.recordTrigger; }
      if (s.vibration && hasVibrate) { dom.vibrationSelect.value = s.vibration; useVibration = s.vibration === 'on'; }
      if (s.touchPressure && hasTouch) { dom.touchPressureSelect.value = s.touchPressure; useTouchPressure = s.touchPressure === 'on'; }
      if (s.micSens) { dom.micSens.value = s.micSens; document.getElementById('val-mic-sens').innerText = s.micSens; }
      if (s.slideMode) { slideMode = s.slideMode; dom.slideModeSelect.value = s.slideMode; }
      if (s.lipMode) { lipMode = s.lipMode; dom.lipModeSelect.value = s.lipMode; }
      if (s.lipDir) { lipDir = s.lipDir; dom.lipDirSelect.value = s.lipDir; }
      if (s.lipSens) { dom.lipSens.value = s.lipSens; document.getElementById('val-lip-sens').innerText = s.lipSens; }
      if (s.yVol) { dom.yVolSelect.value = s.yVol; useYVol = s.yVol === 'on'; }
...
function drawGuides() {
  ...
  hContainer.innerHTML = ''; pContainer.innerHTML = '';
  ...
  for (let i = 0; i < 7; i++) {
    ...
    let leftPercent = slideMode === 'fixed' 
      ? (i / 6) * 100 
...
function createBrassWave(ctx) {
  const real = new Float32Array([0, 1.0, 0.9, 0.8, 0.75, 0.65, 0.6, 0.5, 0.45, 0.35, 0.3, 0.25, 0.2, 0.15, 0.1, 0.05]);
...
function buildAudioEngine() {
  ...
  if (!streamDestination) {
    ...
    mediaRecorder.onstop = () => {
      let ext = 'webm';
      if (!MediaRecorder.isTypeSupported('audio/webm') && MediaRecorder.isTypeSupported('audio/mp4')) ext = 'mp4';
  ...
  if (currentEngine === 'old-real') {
    ...
    bellFilter.type = 'peaking'; bellFilter.frequency.value = 900; bellFilter.Q.value = 1.0; bellFilter.gain.value = 8.0;
    filterNode.Q.value = 0.6; 
    oscillator.connect(bellFilter);
    bellFilter.connect(filterNode);
  } else if (currentEngine === 'real') {
    oscillator.type = 'sawtooth';
    formant1 = audioCtx.createBiquadFilter(); formant1.type = 'peaking'; formant1.frequency.value = 450; formant1.Q.value = 1.0; formant1.gain.value = 4.5;
    formant2 = audioCtx.createBiquadFilter(); formant2.type = 'peaking'; formant2.frequency.value = 1000; formant2.Q.value = 1.2; formant2.gain.value = 5.0;
    filterNode.Q.value = 0.8; 
    oscillator.connect(formant1); formant1.connect(formant2); formant2.connect(filterNode);
  } else {
    oscillator.type = 'sawtooth';
...
async function setupMicrophone(deviceId = null) {
  ...
  const audioSettings = { echoCancellation: false, autoGainControl: false, noiseSuppression: false };
  ...
  try {
    ...
    dom.micSelect.innerHTML = devices
      ...
      .map(d => `<option value="${d.deviceId}" ${d.deviceId === deviceId ? 'selected' : ''}>${d.label || 'マイク'}</option>`)
      .join('');
  } catch (e) {
...
dom.recordBtn.addEventListener('touchstart', handleRecordBtnClick, {passive: false});
...
function updateLoop() {
  ...
  filterNode.frequency.setTargetAtTime(targetCutoff, audioCtx.currentTime, 0.04);
}
...
function handleInput(e) {
  ...
  if (isTouch) {
    for (let i = 0; i < e.touches.length; i++) {
      ...
      if (lipMode === 'pro' && relativeX <= 80) {
        lipTouch = t;
      } else if (!slideTouch && relativeX > 80) {
        slideTouch = t;
  ...
  if (lipMode === 'pro') {
    if (lipTouch) {
      ...
      if (lastLipTouchY !== null) {
        ...
        while (partialAccumulator > 1.0) { requestedPartial++; partialAccumulator -= 1.0; }
        while (partialAccumulator < -1.0) { requestedPartial--; partialAccumulator += 1.0; }
  ...
  if (slideTouch) {
    ...
    if(oscillator) oscillator.frequency.setTargetAtTime(currentFreq, audioCtx.currentTime, 0.03);
    if (inputMode === 'touch' && gainNode) {
      let targetVol = parseFloat(dom.simpleVol.value) / 100;
      let brightnessInput = targetVol; 
      if (useTouchPressure && isTouch && slideTouch.force !== undefined && slideTouch.force > 0) {
      ...
      let targetCutoff;
      if (currentEngine === 'real') {
        targetCutoff = (currentFreq * 1.2) + (Math.pow(brightnessInput, 1.5) * 12000);
      } else if (currentEngine === 'old-real') {
      ...
      targetCutoff = Math.max(10, Math.min(targetCutoff, maxSafeFreq));
      filterNode.frequency.setTargetAtTime(targetCutoff, audioCtx.currentTime, 0.03);
    }
  } else {
    handleStop();
  }
...
function updateUIStates() {
  ...
  dom.statusEngine.style.color = currentEngine === 'real' ? '#4fc3f7' : '#aaa';
  if (inputMode === 'mic') {
    dom.micWrapper.style.opacity = '1'; dom.micWrapper.style.pointerEvents = 'auto';
    dom.statusMode.innerText = 'モード: マイク';
  } else {
    dom.micWrapper.style.opacity = '0.3'; dom.micWrapper.style.pointerEvents = 'none';
    dom.statusMode.innerText = 'モード: シンプル';
    if (gainNode) gainNode.gain.setTargetAtTime(0, audioCtx.currentTime, 0.05);
  }
  if (lipMode === 'pro') {
    dom.lipZone.style.display = 'flex'; dom.guidesContainer.style.left = '80px'; dom.guidesContainer.style.width = 'calc(100% - 80px)';
    dom.proWrapper.style.opacity = '1'; dom.proWrapper.style.pointerEvents = 'auto';
  } else {
    dom.lipZone.style.display = 'none'; dom.guidesContainer.style.left = '0'; dom.guidesContainer.style.width = '100%';
    dom.proWrapper.style.opacity = '0.3'; dom.proWrapper.style.pointerEvents = 'none';
  }
}
...
dom.startOverlay.addEventListener('touchstart', (e) => { e.preventDefault(); initSystem(e); }, {passive: false});
const openSettings = (e) => { e.preventDefault(); e.stopPropagation(); dom.settingsOverlay.style.display = 'flex'; handleStop(); };
dom.openSettingsBtn.addEventListener('mousedown', openSettings);
dom.openSettingsBtn.addEventListener('touchstart', openSettings, {passive: false});
dom.closeSettingsBtn.onclick = () => dom.settingsOverlay.style.display = 'none';
...
dom.engineSelect.onchange = (e) => { currentEngine = e.target.value; if (isInitialized) buildAudioEngine(); updateUIStates(); saveSettings(); };
dom.modeSelect.onchange = async (e) => { inputMode = e.target.value; updateUIStates(); if (inputMode === 'mic' && !isMicInitialized && isInitialized) await setupMicrophone(); saveSettings(); };
dom.recordTriggerSelect.onchange = (e) => { recordTrigger = e.target.value; saveSettings(); };
dom.vibrationSelect.onchange = (e) => { useVibration = e.target.value === 'on'; saveSettings(); };
dom.touchPressureSelect.onchange = (e) => { useTouchPressure = e.target.value === 'on'; saveSettings(); };
dom.yVolSelect.onchange = (e) => { useYVol = e.target.value === 'on'; saveSettings(); };
dom.slideModeSelect.onchange = (e) => { slideMode = e.target.value; drawGuides(); saveSettings(); };
dom.lipModeSelect.onchange = (e) => { lipMode = e.target.value; updateUIStates(); drawGuides(); saveSettings(); };
dom.lipDirSelect.onchange = (e) => { lipDir = e.target.value; saveSettings(); };
dom.micSelect.onchange = (e) => { setupMicrophone(e.target.value); saveSettings(); };
dom.simpleVol.oninput = (e) => { document.getElementById('val-simple-vol').innerText = e.target.value; };
dom.simpleVol.onchange = (e) => { saveSettings(); };
dom.micSens.oninput = (e) => { document.getElementById('val-mic-sens').innerText = e.target.value; };
dom.micSens.onchange = (e) => { saveSettings(); };
dom.lipSens.oninput = (e) => { document.getElementById('val-lip-sens').innerText = e.target.value; };
dom.lipSens.onchange = (e) => { saveSettings(); };
...
playArea.addEventListener('touchstart', (e) => { e.preventDefault(); handleInput(e); }, {passive:false});
playArea.addEventListener('touchmove', (e) => { e.preventDefault(); handleInput(e); }, {passive:false});
playArea.addEventListener('touchend', (e) => { e.preventDefault(); if (e.touches.length === 0) { lastLipTouchY = null; partialAccumulator = 0; handleStop(); } else handleInput(e); }, {passive:false});
playArea.addEventListener('touchcancel', (e) => { e.preventDefault(); lastLipTouchY = null; partialAccumulator = 0; handleStop(); }, {passive:false});
playArea.addEventListener('mousedown', (e) => {
  ...
  const stop = () => { window.removeEventListener('mousemove', move); window.removeEventListener('mouseup', stop); handleStop(); };
...
playArea.addEventListener('wheel', (e) => {
  ...
  if (lipMode === 'pro' && isPlaying) {
    if (e.deltaY < 0) changePartial(currentPartialNum + (lipDir === 'normal' ? 1 : -1)); 
    else changePartial(currentPartialNum + (lipDir === 'normal' ? -1 : 1));
    handleInput(e); 
  }
}, {passive:false});
...
handleResize(); loadSettings(); updateUIStates(); drawGuides(); dom.statusNote.innerText = `音階: -`;
